...
#define SENDER_PUBLICKEY_OFFSET 5U
#define RECIPIENT_OFFSET        38U
#define VENDORFIELD_OFFSET      59U
#define ASSET_OFFSET            139U
...
parserStatus_e parseTxInternal(uint8_t *data, uint32_t length, txContent_t *context) {
    ...
    context->timestamp = (uint32_t)data[4] << 24 |
                         (uint32_t)data[3] << 16 |
                         (uint32_t)data[2] << 8 |
                         (uint32_t)data[1];

    // senderPublicKey / 5..37
    os_memmove(context->senderPublicKey,
               data + SENDER_PUBLICKEY_OFFSET,
               PUBLICKEY_COMPRESSED_LENGTH);

    // recipientId (destination address): decoded base58check / 38..58
    os_memmove(context->recipientId,
               data + RECIPIENT_OFFSET,
               ADDRESS_HASH_LENGTH);

    context->vendorFieldOffset = VENDORFIELD_OFFSET;
    // vendorField: 64 bytes hex / 59..122
    //os_memmove(context->vendorField, data + 59, 64);

    // amount: LE long / 123..130
    // TODO: what about sign
    context->amount = (uint64_t)data[130] << 56 |
                      (uint64_t)data[129] << 48 |
                      (uint64_t)data[128] << 40 |
                      (uint64_t)data[127] << 32 |
                      (uint64_t)data[126] << 24 |
                      (uint64_t)data[125] << 16 |
                      (uint64_t)data[124] << 8 |
                      (uint64_t)data[123];

    // fee: LE long / 131..138
    // TODO: what about sign
    context->fee  = (uint64_t)data[138] << 56 |
                    (uint64_t)data[137] << 48 |
                    (uint64_t)data[136] << 40 |
                    (uint64_t)data[135] << 32 |
                    (uint64_t)data[134] << 24 |
                    (uint64_t)data[133] << 16 |
                    (uint64_t)data[132] << 8 |
                    (uint64_t)data[131];

    // asset, meaning depending on type:
    context->assetlength = length-ASSET_OFFSET;
    context->assetOffset = ASSET_OFFSET;

    // 0: transfer -> no asset
    if(context->type == 0U){
        if(context->assetlength != 0U) {
            return USTREAM_FAULT;
        }
    }
    // 1: second signature -> publicKey (of the secondsignature) 139..171
    else if(context->type == 1U){
        if(context->assetlength != PUBLICKEY_COMPRESSED_LENGTH) {
            return USTREAM_FAULT;
        }
        //os_memmove(context->asset, data + 139, 33);
    }
    // 2: delegate registration -> username serialized in utf8
    else if(context->type == 2U){
        if(context->assetlength > 40) {
            return USTREAM_FAULT;
        }
        //os_memmove(context->asset, data + 139, context->assetlength);
    }
    // 3: vote -> publicKeys serialized in utf8 (for instance "+0345.....")
    else if(context->type == 3U){
        // context->assetlength should be a multiple of (33*2 + 1) = 67
        if(context->assetlength % 67 != 0) {
            return USTREAM_FAULT;
        }
    ...
    // 5: ipfs -> no asset
    else if(context->type == 5U){
        if(context->assetlength > 0) {
            return USTREAM_FAULT;
        }
    }
